...
echo This is a basic Ubunutu/apt lockdown script, however based upon what box you are using,
echo you may need to change the firewall ports being opened/closed! - LINE 35 - 78 - there are templates there!
...
read -p "Press enter to continue and setup firewall"
...
#Setting connections to drop unless allowed
		...
		#firewall-cmd --set-default-zone=drop - this will block all incoming unless allowed, could cause scoring issues
		firewall-cmd --reload
# Adding Rules - UNCOMMENT TO USE!!
...
#blocking rules - uncomment to use these!
...
echo $(date): using chattr +i to lock files >> /var/log/ryanlog.log
	...
	#chattr -R +i /PATH/TO/SERVICE-or-file
	# -R means recursive, +i locks it down - to undo: chattr -R -i /PATH
